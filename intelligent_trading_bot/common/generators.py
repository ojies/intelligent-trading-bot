...
def predict_feature_set(df, fs, config, model_store: ModelStore) -> Tuple[pd.DataFrame, list]:
    ...
    for label in labels:
        for model_config in algorithms:
            ...
            if algo_type == "gb":
                from intelligent_trading_bot.common.classifier_gb import predict_gb
                df_y_hat = predict_gb(model_pair, train_df, model_config)
            elif algo_type == "nn":
                from intelligent_trading_bot.common.classifier_nn import predict_nn
                df_y_hat = predict_nn(model_pair, train_df, model_config)
            elif algo_type == "lc":
                from intelligent_trading_bot.common.classifier_lc import predict_lc
                df_y_hat = predict_lc(model_pair, train_df, model_config)
            elif algo_type == "svc":
                from intelligent_trading_bot.common.classifier_svc import predict_svc
...
def train_feature_set(df, fs, config) -> dict:
    ...
    for label in labels:
        for model_config in algorithms:
            ...
            if algo_type == "gb":
                from intelligent_trading_bot.common.classifier_gb import train_gb
                model_pair = train_gb(df_X, df_y, model_config)
                models[score_column_name] = model_pair
            elif algo_type == "nn":
                from intelligent_trading_bot.common.classifier_nn import train_nn
                model_pair = train_nn(df_X, df_y, model_config)
                models[score_column_name] = model_pair
            elif algo_type == "lc":
                from intelligent_trading_bot.common.classifier_lc import train_lc
                model_pair = train_lc(df_X, df_y, model_config)
                models[score_column_name] = model_pair
            elif algo_type == "svc":
                from intelligent_trading_bot.common.classifier_svc import train_svc
...
async def output_feature_set(df, fs: dict, config: dict, model_store: ModelStore) -> None:
    from intelligent_trading_bot.outputs.notifier_scores import send_score_notification
    from intelligent_trading_bot.outputs.notifier_diagram import send_diagram
    from intelligent_trading_bot.outputs.notifier_trades import trader_simulation
